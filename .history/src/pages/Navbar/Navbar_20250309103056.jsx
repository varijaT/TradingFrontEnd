      ...
      <div className="px-2 py-3 border-b z-50 bg-background sticky top-0 left-0 right-0 flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <Sheet open={isSidebarOpen} onOpenChange={setIsSidebarOpen}>
            <SheetTrigger asChild>
              <Button className="rounded-full h-11 w-11 lg:hidden" variant="ghost" size="icon">
        ...
        <div>
          <Avatar className="cursor-pointer" onClick={handleNavigate}>
            {!auth.user ? <AvatarFallback>U</AvatarFallback> : <AvatarFallback>{auth.user?.fullName[0].toUpperCase()}</AvatarFallback>}
          </Avatar>
        </div>
      </div>
    </>
